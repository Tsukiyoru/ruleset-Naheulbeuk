...
function setLocalNode(node)
	...
	if node then
		if node.isStatic() then
			campaign.setValue(Interface.getString("charselect_label_server"));
		else
			campaign.setValue(Interface.getString("charselect_label_local"));
			registerMenuItem(Interface.getString("char_menu_delete"), "delete", 3);
			registerMenuItem(Interface.getString("list_menu_deleteconfirm"), "delete", 3, 4);
