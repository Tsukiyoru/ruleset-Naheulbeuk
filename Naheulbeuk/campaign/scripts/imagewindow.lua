...
function showDataWindow()
	...
	Interface.openWindow("imagedatawindow", getDatabaseNode());
end

function hideDataWindow()
	if Interface.getVersion() ~= 4 then return; end
	local w = Interface.findWindow("imagedatawindow", getDatabaseNode());
	if w then
		w.close();
	end
end
...
function updateDisplay()
	if Interface.getVersion() ~= 3 then return; end
