...
function initialize()
	...
	for kRecordType,vRecord in pairs(aRecords) do
		...
		if aMappings then
			...
			if vRecord.bExport then
				if #aMappings > 0 then
					if vRecord.bSkipExportList then
						ExportManager.registerExportNode({ name = kRecordType, source = aMappings[1], export = aMappings[1], exportref = aMappings[2], class = "", label = vRecord.sDisplayText});
					else
						ExportManager.registerExportNode({ name = kRecordType, source = aMappings[1], export = aMappings[1], exportref = aMappings[2], class = sDisplayClass, label = vRecord.sDisplayText});
					end
				end
			-- Images should always be editable to be able to apply masks, grids, etc.
			elseif vRecord.bExportNoReadOnly then
				if #aMappings > 0 then
					if vRecord.bSkipExportList then
						ExportManager.registerExportNode({ name = kRecordType, source = aMappings[1], export = aMappings[1], exportref = aMappings[1], class = "", label = vRecord.sDisplayText});
					else
						ExportManager.registerExportNode({ name = kRecordType, source = aMappings[1], export = aMappings[1], exportref = aMappings[1], class = sDisplayClass, label = vRecord.sDisplayText});
					end
				end
			elseif vRecord.nExport then
				...
				if #aExportMappings > 0 then
					if vRecord.bSkipExportList then
						ExportManager.registerExportNode({ name = kRecordType, source = aExportMappings, export = aExportMappings, exportref = aExportRefMappings, class = "", label = vRecord.sDisplayText});
					else
						ExportManager.registerExportNode({ name = kRecordType, source = aExportMappings, export = aExportMappings, exportref = aExportRefMappings, class = sDisplayClass, label = vRecord.sDisplayText});
					end
				end
			end
		end
...
function getIDState(sRecordType, vNode, bIgnoreHost)
	...
	if isIdentifiable(sRecordType, vNode) then
		if aRecords[sRecordType].fGetIDState then
			bID, bOptionID = aRecords[sRecordType].fGetIDState(vNode, bIgnoreHost);
		else
			bOptionID = true;
			local sInternalIDOption = getIDOption(sRecordType);
			if sInternalIDOption ~= "" then
				bOptionID = OptionsManager.isOption(sInternalIDOption, "on");
			end
			if bOptionID and (bIgnoreHost or not User.isHost()) then
				bID = (DB.getValue(vNode, "isidentified", 0) == 1);
			end
		end
	end
