...
function requestSelection(sTitle, sMsg, aSelections, fnCallback, vCustom, nMinimum, nMaximum)
...
function processOK()
	if #aQueue > 0 then
		if aQueue[1].callback then
			...
			aQueue[1].callback(aSelections, aQueue[1].custom);
		end
